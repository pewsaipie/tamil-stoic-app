...
async function shareQuote(quote) {
  ...
  ctx.fillStyle = '#5b2029'; ctx.fillRect(0, 0, canvas.width, canvas.height); ctx.strokeStyle = '#d8ac5b'; ctx.lineWidth = 8; ctx.strokeRect(36, 36, canvas.width - 72, canvas.height - 72); ctx.strokeStyle = '#a96a43'; ctx.lineWidth = 2; ctx.strokeRect(58, 58, canvas.width - 116, canvas.height - 116);
  ctx.textAlign = 'center'; ctx.fillStyle = '#e7bf6e'; ctx.font = '52px Georgia'; ctx.fillText('✦  ❖  ✦', 540, 190); ctx.fillStyle = '#fff5df'; ctx.font = '42px Noto Serif Tamil, serif'; const tamilLines = quote.tamil.text.split('\n'); tamilLines.forEach((line, i) => ctx.fillText(line, 540, 410 + i * 76)); ctx.fillStyle = '#e7bf6e'; ctx.fillRect(500, 570, 80, 4); ctx.fillStyle = '#fff5df'; ctx.font = 'italic 31px Georgia'; const words = easy.split(' '); let line = '', y = 705; words.forEach(word => { if ((line + ' ' + word).length > 48) { ctx.fillText(line, 540, y); y += 45; line = word; } else line += `${line ? ' ' : ''}${word}`; }); if (line) ctx.fillText(line, 540, y); ctx.fillStyle = '#ead5a4'; ctx.font = '21px Georgia'; ctx.fillText(`${quote.work.title} · ${quote.source_locator}`, 540, 1060); ctx.font = '18px Manrope, sans-serif'; ctx.fillText('Tamil Stoic · source cited in app', 540, 1140);
  canvas.toBlob(async blob => { const file = new File([blob], `${quote.id}.png`, { type: 'image/png' }); try { if (navigator.canShare?.({ files: [file] })) await navigator.share({ title: `Tamil Stoic · ${quote.source_locator}`, text, files: [file] }); else { const link = document.createElement('a'); link.href = URL.createObjectURL(blob); link.download = `${quote.id}-tamil-stoic.png`; link.click(); URL.revokeObjectURL(link.href); await navigator.clipboard?.writeText(text); toast('Decorated quote card downloaded and citation copied.'); } } catch (error) { if (error.name !== 'AbortError') toast('Sharing was not available.'); } }, 'image/png');
}
