 ...
 * Modern plain-English simple meanings for all other 1,301 kurals are merged
 * from scripts/glosses/part-*.json (app-original, authored from Pope's prose;
 * curated kurals are never touched — they were excluded at authoring time).
 *
...
/* ---------- modern plain-English glosses (scripts/glosses/part-*.json) ---------- */

const glosses = new Map();
const glossDir = path.join(here, "glosses");
for (const f of fs.readdirSync(glossDir).filter((f) => f.endsWith(".json")).sort()) {
  const batch = JSON.parse(fs.readFileSync(path.join(glossDir, f), "utf8"));
  for (const [num, text] of Object.entries(batch)) {
    const n = Number(num);
    if (!Number.isInteger(n) || n < 1 || n > 1330) throw new Error(`${f}: bad kural number "${num}"`);
    if (glosses.has(n)) throw new Error(`${f}: duplicate gloss for kural ${n}`);
    if (curated[String(n)]) throw new Error(`${f}: kural ${n} is curated — glosses must not touch it`);
    if (typeof text !== "string" || !text.trim()) throw new Error(`${f}: empty gloss for kural ${n}`);
    glosses.set(n, text.trim());
  }
}
...
for (let n = 1; n <= 1330; n++) {
  ...
  if (c) {
    ...
    if (g) {
      ...
      popeFallback++;
      console.warn(`WARN: kural ${n} has no modern gloss — falling back to Pope prose`);
    }
  }
 ...
 *
 * Tamil couplets: standard (Parimelalagar-based) recension. Tamil text,
 * transliterations and chapter structure derived from the
 * tk120404/thirukkural dataset (https://github.com/tk120404/thirukkural),
 * licensed under the Apache License 2.0.
 * English verse translations and prose explanations: G. U. Pope with
 * Drew, Lazarus & Ellis, 1886 — public domain.
 * Simple-English meanings are original to this app: the 29 curated entries
 * (scripts/curated-overrides.json) and the modern glosses merged from
 * scripts/glosses/*.json, authored from Pope's prose.
...
console.log(`glosses merged : ${glossesApplied} (modern plain-English)`);
console.log(`pope fallback  : ${popeFallback}`);
